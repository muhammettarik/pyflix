...
class seriesDetailed:
    def __init__(self, mid, parent, user):
        ...
        if settings.language =="en":
            ...
            self.get_movie_detailTr()   

    def get_movie_detail(self):
        con = sqlite3.connect("Series.db")
        cur = con.cursor()
        cur.execute("SELECT mid, name, category, subject, release, imdb_rate FROM Series WHERE mid=?", [self.mid])
        rec = cur.fetchone()
        con.close()
        self.fill_window(rec)

    def get_movie_detailTr(self):
        con = sqlite3.connect("Series.db")
        cur = con.cursor()
        cur.execute("SELECT mid, name, category, subject, release, imdb_rate FROM SeriesTr WHERE mid=?", [self.mid])
        rec = cur.fetchone()
        con.close()
        self.fill_window(rec)
    ...
    def addFavorite(self):
        ...
        cur.execute("UPDATE Users SET favs=:fav WHERE username=:user",
